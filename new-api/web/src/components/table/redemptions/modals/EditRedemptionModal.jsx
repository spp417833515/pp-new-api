...
const EditRedemptionModal = (props) => {
  ...
  const submit = async (values) => {
    // 将美元转换为配额
    const quota = usdToQuota(parseFloat(values.quotaUsd) || 0);
    ...
    localInputs.quota = quota;
    delete localInputs.quotaUsd;
                  ...
                  <Row gutter={12}>
                    <Col span={12}>
                      <Form.InputNumber
                        ...
                        extraText={`${t('配额')}: ${usdToQuota(Number(values.quotaUsd) || 0)}`}
                        showClear
                      />
                    </Col>
                    {!isEdit && (
