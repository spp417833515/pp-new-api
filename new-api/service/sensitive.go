...
// CheckSensitiveText 检查文本是否包含敏感词，返回是否触发阈值和匹配的敏感词列表
func CheckSensitiveText(text string) (bool, []string) {
	return SensitiveWordContains(text)
}

// SensitiveWordContains 是否包含敏感词，返回是否触发阈值和敏感词列表
// 当匹配次数 >= SensitiveWordsThreshold 时返回 true
func SensitiveWordContains(text string) (bool, []string) {
	...
	// 使用 stopImmediately=false 获取所有匹配结果
	found, words := AcSearch(checkText, setting.SensitiveWords, false)
	if !found {
		return false, nil
	}
	// 检查匹配次数是否达到阈值
	threshold := setting.SensitiveWordsThreshold
	if threshold <= 0 {
		threshold = 1
	}
