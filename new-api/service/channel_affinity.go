...
const (
	...
	channelAffinityFailCacheNamespace = "new-api:channel_affinity_fail:v1"
)
...
var (
	...
	channelAffinityFailCacheOnce sync.Once
	channelAffinityFailCache     *cachex.HybridCache[int]
...
func getChannelAffinityFailCache() *cachex.HybridCache[int] {
	channelAffinityFailCacheOnce.Do(func() {
		setting := operation_setting.GetChannelAffinitySetting()
		capacity := setting.MaxEntries
		if capacity <= 0 {
			capacity = 100_000
		...
		channelAffinityFailCache = cachex.NewHybridCache[int](cachex.HybridCacheConfig[int]{
			Namespace: cachex.Namespace(channelAffinityFailCacheNamespace),
			Redis:     common.RDB,
			RedisEnabled: func() bool {
				return common.RedisEnabled && common.RDB != nil
			},
	...
	return channelAffinityFailCache
}
...
func RecordChannelAffinity(c *gin.Context, channelID int) {
	...
	// 成功时清除失败计数
	ClearAffinityFailCount(c)
}

// IsAffinityFailureStatus 判断状态码是否属于渠道失败
func IsAffinityFailureStatus(statusCode int) bool {
	setting := operation_setting.GetChannelAffinitySetting()
	if setting == nil || !setting.Enabled || setting.MaxFailures <= 0 {
		return false
	}
	failureCodes := setting.FailureStatusCodes
	if len(failureCodes) == 0 {
		// 默认失败状态码
		failureCodes = []int{429, 500, 502, 503, 504}
	}
...
// IncrementAffinityFail 增加失败计数，达到阈值时清除亲和性
func IncrementAffinityFail(c *gin.Context) {
	setting := operation_setting.GetChannelAffinitySetting()
	if setting == nil || !setting.Enabled || setting.MaxFailures <= 0 {
		return
	}
	...
	failCache := getChannelAffinityFailCache()
	// 获取当前失败计数
	currentCount, _, _ := failCache.Get(cacheKey)
	newCount := currentCount + 1

	if newCount >= setting.MaxFailures {
		// 达到阈值，清除亲和性缓存和失败计数
		...
		if _, err := failCache.DeleteMany([]string{cacheKey}); err != nil {
			common.SysError(fmt.Sprintf("channel affinity fail cache delete failed: key=%s, err=%v", cacheKey, err))
		}
		common.SysLog(fmt.Sprintf("channel affinity cleared due to %d consecutive failures: key=%s", newCount, cacheKey))
	} else {
		// 未达到阈值，更新失败计数
		if err := failCache.SetWithTTL(cacheKey, newCount, time.Duration(ttlSeconds)*time.Second); err != nil {
			common.SysError(fmt.Sprintf("channel affinity fail cache set failed: key=%s, err=%v", cacheKey, err))
		}
	}
}

// ClearAffinityFailCount 清除失败计数
func ClearAffinityFailCount(c *gin.Context) {
	setting := operation_setting.GetChannelAffinitySetting()
	if setting == nil || !setting.Enabled || setting.MaxFailures <= 0 {
		return
	}
	...
	failCache := getChannelAffinityFailCache()
	// 忽略删除错误，因为可能本来就不存在
	_, _ = failCache.DeleteMany([]string{cacheKey})
}
