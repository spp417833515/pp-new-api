...
func StartLogAutoCleanTask() {
	for {
		if !common.LogAutoCleanEnabled {
			time.Sleep(time.Minute)
			continue
		}

		intervalMinutes := common.LogCleanIntervalMinutes
		...
		deleted, err := CleanLogByCount(maxCount)
		if err != nil {
			common.SysError(fmt.Sprintf("auto clean log error: %v", err))
		} else if deleted > 0 {
			common.SysLog(fmt.Sprintf("auto clean log: deleted %d records, max count %d", deleted, maxCount))
		}
	}
}
